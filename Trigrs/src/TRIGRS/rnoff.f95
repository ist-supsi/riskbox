...
! By Rex L. Baum, 1 April 2004	
	...
	integer:: i,j,l,m,imx1,roflg,mnd
	integer:: grd,unum
...
!  read numbers of subjacent cells
	call irdgrd(grd,col,ncol,nrow,celsiz,nodata,mnd,&
     &	nxt,pf2,sctr,imax,temp,u(17),nxtfil,parami,header,u(19))
...
! save infiltration rate for grids that have non-zero runoff	  
	    if (outp(6)) then
	      ...
	      outfil=trim(folder)//trim(irfil)
   	      call ssvgrd(ir,imax,pf1,nrow,ncol,u(6),test1,param,u(19),&
     &	      outfil,ti,header)
	    end if
	   end if
	  end if  
