...
! uses analytic solution for transmissivity
	...
	Z_loop: do j=1,nzs+1
	  ...
	  t_loop: do m=1,iper
		...
		cn=float(2*m-1)	
	        ...
	        f1=f1a-f1b*f1c
        	ka=ck0+2.*f1*f0 
        	trva=ka*zinc ! approximate formula for transmissivity
		...
		f2=f2a-f2b*f2c
        	kb=ck0+2.*f2*f0 
        	trvb=ka*zinc ! approximate formula for transmissivity
	      else ! later-time series solution
	        k2b=0.d0; trb=0.d0
	  ...
	  if(t1==0.) then
	    kz(j)=ck0
      	    ptran(j)=psi0*b1
! Analytic formulas for transmissivity, trz()      	    
      	    if(lps0) then 
...
! Numerical integration of transmissivity (trvty)
	trvty=-(zmax(i)-d)*cks 
	do j=nzs,1,-1
