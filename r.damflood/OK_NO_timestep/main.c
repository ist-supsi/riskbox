...
int main(int argc, char *argv[]){
 ...
 /* typedef struct
  ...
  int outfd_H,outfd_VEL,outfd_VEL_DIR,outfd_HMAX,outfd_T_HMAX,outfd_I_HMAX,outfd_VMAX,outfd_T_VMAX,outfd_I_VMAX,outfd_DIR_VMAX,outfd_IMAX,outfd_T_IMAX;
  float g=9.81;
  /* mapset name locator */
  ...
  int method;
  int warn1=0,warn2=0;
  double Q=0.0, vol_res,fall, volume=0.0;
  float res_ew ,res_ns;
  float R_i;
  /* memory matrix */
  double **m_h1, **m_h2, **m_u1, **m_u2, **m_v1, **m_v2;
  float **m_z, **m_DAMBREAK, **m_m;
  float **m_hmax, **m_t_hmax, **m_i_hmax, **m_vmax, **m_t_vmax, **m_i_vmax, **m_dir_vmax, **m_imax, **m_t_imax ;
  int ** m_lake;
  /*  other variables  */
  ...
  char *ELEV, *LAKE, *DAMBREAK, *MANNING, *OUT_VEL, *OUT_H, *OUT_HMAX, *OUT_VMAX, *OUT_IMAX;
  ...
  struct
    ...
    parm;
  struct Option *output_VEL,*output_H,*output_HMAX, *output_VMAX, *output_IMAX;
  struct Flag *flag_d;
  struct {
		struct Option *met;
	} opt;
  ...
  */
  ...
  input_TIMESTEP->description = _("Computational time step [s] - CFL condition");
  ...
  G_close_cell (infd_MANNING);
	...
	/*************************************************************************************************************************************************
					...
					if (m_DAMBREAK[row-1][col]!=0 && m_h2[row-1][col]==0){
						dZ_dy_up=0.0;
						dZ_dy_down = ((m_h2[row][col] + m_z[row][col]) - (m_h2[row+1][col] + m_z[row+1][col]) ) / res_ns;
					}
					if (m_DAMBREAK[row+1][col]!=0 && m_h2[row+1][col]==0){
...
//*************************************** overwriting *********************************************
    ...
    /* open new cicle */
      ...
      /* if T e' piu' o meno multiplo di DELTAT allora scrivi output con istante */
     	...
     	if (((m*DELTAT-t) <= timestep && m*DELTAT < TSTOP || ( ( pp<ntimes && (times[pp]-t) < timestep) && (parm.opt_t->answer != NULL) ))
