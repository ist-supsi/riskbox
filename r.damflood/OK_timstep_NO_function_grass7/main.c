...
int main(int argc, char *argv[]){
  ...
  void *inrast_ELEV;			/* elevation map [m]*/
  void *inrast_LAKE;			/* water elevation in the map [m]*/
  void *inrast_DAMBREAK;		/* break in the dam*/
  void *inrast_MANNING;		/* manning*/
  void *outrast_VEL;		   /* velocity [m/s] */
  void *outrast_VEL_DIR;
  void *outrast_H;	         /* water elevation output [m]*/
  void *outrast_HMAX;
  void *outrast_I_HMAX;
  void *outrast_T_HMAX;
  void *outrast_VMAX;
  void *outrast_I_VMAX;
  void *outrast_T_VMAX;
  void *outrast_DIR_VMAX;
  void *outrast_IMAX;
  void *outrast_T_IMAX;
  void *outrast_WAVEFRONT;
  /* cell counters */
  int nrows, ncols;
  int row, col;
...
//*************************************** overwriting *********************************************
	...
	if (( ((m*DELTAT-t) <= timestep) && (m*DELTAT < TSTOP)&& (input_DELTAT->answer != NULL)) || ( (pp<ntimes && (times[pp]-t) < timestep) && (parm.opt_t->answer != NULL))) {
		//G_message("pippo");
			...
			if (OUT_H) {
				// debug monia: Rast_open_new(result_H,Rast_map_type(name_HINI,mapset_HINI));
				// mio vecchio: if ( (outfd_H = G_open_raster_new (name1,DCELL_TYPE)) < 0)
