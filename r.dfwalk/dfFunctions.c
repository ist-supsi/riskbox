...
//printf("\n- calculated gamma_max^alfa = %lf", pow(gamma_max,a) );
...
//printf("\n- count selectable cells = %d",count);
    ...
    /* allocate array */
    selected_idx = (int*)G_malloc(sizeof(int)*(count+1));
    probability = (double*)G_malloc(sizeof(double)*(count+1));
    upper_bound = (double*)G_malloc(sizeof(double)*(count+1));
