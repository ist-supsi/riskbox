...
/* allocate a CELL matrix with subscript range m[nrl..nrh][ncl..nch] */
CELL **Cmatrix(int nrl, int nrh, int ncl, int nch)
{
	int i, nrow=nrh-nrl+1,ncol=nch-ncl+1;
	CELL **m;

	/* allocate pointers to rows */
	m = (CELL **) G_malloc (((nrow+NR_END) * sizeof(CELL*)));
	m += NR_END;
	m -= nrl;

	/* allocate rows and set pointers to them */
	m[nrl] = (CELL *) G_malloc (((nrow*ncol+NR_END) * sizeof(CELL)));
	m[nrl] += NR_END;
	m[nrl] -= ncl;
...
/* allocate a FCELL matrix with subscript range m[nrl..nrh][ncl..nch] */
FCELL **Fmatrix(int nrl, int nrh, int ncl, int nch)
{
	int i, nrow=nrh-nrl+1,ncol=nch-ncl+1;
	FCELL **m;

	/* allocate pointers to rows */
	m = (FCELL **) G_malloc (((nrow+NR_END) * sizeof(FCELL*)));
	m += NR_END;
	m -= nrl;

	/* allocate rows and set pointers to them */
	m[nrl] = (FCELL *) G_malloc (((nrow*ncol+NR_END) * sizeof(FCELL)));
	m[nrl] += NR_END;
	m[nrl] -= ncl;
...
/* allocate a FCELL matrix with subscript range m[nrl..nrh][ncl..nch] */
DCELL **Dmatrix(int nrl, int nrh, int ncl, int nch)
{
	int i, nrow=nrh-nrl+1,ncol=nch-ncl+1;
	DCELL **m;

	/* allocate pointers to rows */
	m = (DCELL **) G_malloc (((nrow+NR_END) * sizeof(DCELL*)));
	m += NR_END;
	m -= nrl;

	/* allocate rows and set pointers to them */
	m[nrl] = (DCELL *) G_malloc (((nrow*ncol+NR_END) * sizeof(DCELL)));
	m[nrl] += NR_END;
	m[nrl] -= ncl;
...
/* free a FCELL matrix allocated by matrix() */
void free_Fmatrix(FCELL **m, int nrl, int nrh, int ncl, int nch)
{
	G_free ((FREE_ARG) (m[nrl]+ncl-NR_END));
	G_free ((FREE_ARG) (m+nrl-NR_END));
}

/* free a CELL matrix allocated by matrix() */
void free_Dmatrix(DCELL **m, int nrl, int nrh, int ncl, int nch)
{
	G_free ((FREE_ARG) (m[nrl]+ncl-NR_END));
	G_free ((FREE_ARG) (m+nrl-NR_END));
}

void free_Cmatrix(CELL **m, int nrl, int nrh, int ncl, int nch)
{
	G_free ((FREE_ARG) (m[nrl]+ncl-NR_END));
	G_free ((FREE_ARG) (m+nrl-NR_END));
}
