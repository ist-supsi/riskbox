...
int main(int argc, char *argv[])
{
    ...
    /* flag descriptors */		
    int verbose, maxvel, number_rw, input_zone;
    ...
    flag4 = G_define_flag();
    flag4->key = 'i';
    flag4->description = _("Use the input zone file as Volume [m3]");
    ...
    rw_start = (rw_point *)malloc(sizeof(rw_point) * START_DIM );
	...
	for (col = 0; col < ncols; col++) {
            ...
            if( ((DCELL *) S_start.in_buf)[col] != 0 ) {
                if ( (n_start!=0) && (n_start % START_DIM)==0 ) {
                    ia++;
                    //tmp_start = (rw_point *)G_realloc(rw_start, sizeof(rw_point) * START_DIM *ia );
                    tmp_start = (rw_point *)realloc(rw_start, sizeof(rw_point) * START_DIM *ia );
                    if (tmp_start != NULL ) { rw_start = tmp_start; }
                }
                rw_start[n_start].r = row;
                rw_start[n_start].c = col;
                rw_start[n_start].vol = ((DCELL *) S_start.in_buf)[col];
                //printf("rw_start[%d].vol=%f",n_start,rw_start[n_start].vol);
                if (input_zone){
                	vol_tot += rw_start[n_start].vol;
                }
                //printf("n_start=%d - vol_tot=%f",n_start,vol_tot);
                n_start++;
            //} else {
            //	m++;
            //	printf("ecco!%d\n");
...
n_start_ctrl = n_start;
	...
	if (number_rw) {
		...
		if (n_start_ctrl==0) {rw_stop=1;}
	} else {
		if (nrw == (5*i_rw) || Vol >= vol_tot) { rw_stop=1; }	
	}
	...
	for(m=0;m<n_start;m++){
		...
		//while(!getchar()){ }
			...
			cell_path = (rw_cell *)malloc(sizeof(rw_cell) * lenght);
			if (cell_path == NULL) {
			...
			//printf("\Memory lenght= %d * %d",sizeof(rw_cell), lenght);
			/* calculate the singles cells path */        
			while (stop != TRUE) { 
			//printf("\nstep:%d, lenght=%d",step,lenght);
			//while(!getchar()){ }          
			...
			if ( (step!=0) && ((step+1) % lenght)==0 ) {
		    		...
		    		//printf("\nreallocation memory");
		    		//while(!getchar()){ }
		    		tmp_rw = (rw_cell  *)realloc(cell_path, sizeof(rw_cell ) * lenght * ia);
		    		if (tmp_rw == NULL) {
		    			/* la realloc fallisce (e restituisce un puntatore null), ma il puntatore originale ptr è andato perduto 
    					quindi la memoria non può essere liberata e avviene una perdita di memoria */
		    			printf("memoria non allocata");
				...
				case 1: /* deterministic path: the package follows the max slope */
					...
					if(cell_path[step+1].Pend < cell_path[step].Pend) {
						v_star = cell_path[step].Vel * cos(beta-beta_prev);
				...
				case 2: /* stocastic path: the package follows a statistica extraction */
					...
					//if (step==0 && vel_next==0) {
					//	printf("beta=%f,beta_prev=%f, alpha=%f\n",beta, beta_prev, alpha);
					//	printf("1-exp(teta)=%f\n",(1-exp(teta)));
					//	printf("step_lenght=%f,teta=%f, v_star=%f\n",step_lenght, teta, v_star);
			...
			for(j=1;j<step;j++) {
				...
				//if (input_zone) {			   
				//	sed_hmax = min_among_2( (sed_0hmax / (S_dem.cellhd.ew_res*S_dem.cellhd.ns_res)),((rw_start[m].vol+0.01) / (S_dem.cellhd.ew_res*S_dem.cellhd.ns_res)));
			...
			if (input_zone){
				/*if (sed_tot==0 && step>1) {
					G_warning ("ctrl=%d --> sed_tot[%d]=0\n",ctrl,m);
					printf("step=%d", step-1);
					printf("vel_next=%f", vel_next);
					while(!getchar()){ };
				}*/
				rw_start[m].vol = rw_start[m].vol - min_among_2(sed_hmax,sed_tot)*(S_dem.cellhd.ew_res*S_dem.cellhd.ns_res);
				if (rw_start[m].vol < sed_hmax *(S_dem.cellhd.ew_res*S_dem.cellhd.ns_res) ){
					n_start_ctrl = n_start_ctrl-1;
					//printf("n_start_ctrl=%d, rw_start[%d].vol=%f\n",n_start_ctrl,m,rw_start[m].vol);
					//while(!getchar()){ };
					rw_start[m].vol = 0;
				}				
